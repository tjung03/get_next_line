...
**파일 디스크립터에서 텍스트를 한 줄씩 읽는 C 함수입니다.** `read()`로 읽은 데이터를 누적하고, 개행 뒤에 남은 문자열을 다음 호출까지 보관합니다. 42 Seoul 프로젝트로 작성했으며 파일별 읽기 상태를 정적 배열로 관리합니다. 파일 디스크립터(FD)는 열린 파일이나 표준 입력 등을 구분하는 정수 식별자입니다.
